...
def get_original_classification_head(args):
    filename = os.path.join(
        args.model_ckpt_dir,
        args.model,
        f'head_{args.dataset_name}.pt',
    )
    if not os.path.exists(filename):
        print(f'Did not find classification head for {args.model} on {args.dataset_name} at {filename}, building one from scratch.')
        model = ImageEncoder(args, keep_lang=True).model
        template = get_templates(args.dataset_name)
